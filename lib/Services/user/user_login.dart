...
Future<void> loginUser({
  ...
  required String password,
  required String phoneNumber,
  void Function(int userId)? onSuccess, // Add a callback function to receive userId
  String? restaurantId,
}) async {
  final Map<String, dynamic> userData = {
    "password": password,
  ...
  try {
    ...
    if (body['success'] == true) {
      ...
      if (role == 'user' || role == 'restaurant' || role == 'admin') {
        int userId = body['data']['userid']; // Extract userId
        // Save user data to shared preferences
        SharedPreferences prefs = await SharedPreferences.getInstance();
        prefs.setString('role', role);
        prefs.setString('restoName', body['data']['username']);
        ...
        // Call the onSuccess callback with the userId
        if (onSuccess != null) {
          onSuccess(userId);
        }
        ...
        if (role == 'user') {
          Navigator.pushReplacement(
              context, MaterialPageRoute(builder: (context) => UserHomePage())); // Pass userId to UserHomePage
        } else if (role == 'restaurant') {
          //String restaurantId = body['data']['restaurant_id']; // Extract restaurant ID
          Navigator.pushReplacement(
              context, MaterialPageRoute(builder: (context) => ManageFoodPage()));
        } else if (role == 'admin') {
          // Navigate to the admin home page (replace with your admin page)
          Navigator.pushReplacement(
